...
POLYVORE_PRECOMPUTED_CLIP_EMBEDDING_DIR = "{polyvore_dir}/precomputed_text_embeddings"
...
def setup_dataloaders(rank, world_size, args):
    item_dataset = polyvore.PolyvoreTextDataset(
...
def compute(rank: int, world_size: int, args: Any):  
    ...
    # Logging Setup
    logger = get_logger('precompute_text_embedding', LOGS_DIR, rank)
    ...
    with torch.no_grad():
        for batch in tqdm(item_dataloader):
            ...
            if dist.get_world_size() > 1:
                embeddings = model.module.precompute_text_embedding(batch)  # (batch_size, d_embed)
            else:
                print("2")
                embeddings = model.precompute_text_embedding(batch)
